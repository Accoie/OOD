...
namespace MultiGumballMachineApp.Tests.StatesTests
{
    ...
    public class SoldOutStateTests
    {
        ...
        {
            // Act
            _soldOutState.RefillGumballs( 0 );
        ...
        [Test]
        public void RefillGumballs_WithFiveGumballsAndQuarters_SetCorrectState()
        {
            // Arrange
            _gumballMachineMock.Setup( g => g.GetQuartersCount() ).Returns( 5 );

            // Act
            _soldOutState.RefillGumballs( 5 );
            string output = _stringWriter.ToString().Trim();

            // Assert
            _gumballMachineMock.Verify( g => g.SetHasQuarterState(), Times.Once );
            Assert.That( output, Is.EqualTo( "Gumballs was changed on 5" ) );
        }

        [Test]
        public void RefillGumballs_WithoutQuarters_SetCorrectState()
        {
            // Arrange
            _gumballMachineMock.Setup( g => g.GetQuartersCount() ).Returns( 0 );
            ...
            // Assert
            _gumballMachineMock.Verify( g => g.SetNoQuarterState(), Times.Once );
            Assert.That( output, Is.EqualTo( "Gumballs was changed on 5" ) );
        }
